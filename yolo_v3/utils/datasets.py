...
def horisontal_flip(images, targets):
    #수평 방향 뒤집기
    images = torch.filp(images, [-1])
    targets[:,2] = 1 - targets[:,2] #x좌표
    return images, targets

def resize(image, size):
    return F.interpolate(image.unsqueeze(0), size, mode='bilinear', align_corners=True).sequeeze(0)
...
class ListDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        #1. image
        image_path = self.image_files[index].rstrip() #오른쪽 공백 제거

        if self.augment:
            transforms = torchvision.transforms.Compose([
                torchvision.transforms.ColorJitter(brightness=1.5, saturation=1.5, hue=1.0),
        ...
        if os.path.exists(label_path):
            ...
            boxes = torch.from_numpy(np.loadtxt(label_path).reshape(-1,5)) #좌표가 1이내 #shape(n, 5)
            #이미지 크기로 만들기
            ...
            y1 += pad[2]
            y2 += pad[3]
            # returns class label + normalized padding image(x_ctr, y_ctr, w, h) -> 1:1 크기에서
            boxes[:,1] = ((x1+x2)/2)/padded_w
            boxes[:, 2] = ((y1 + y2) / 2) / padded_h
            boxes[:, 3] *= w_factor / padded_w  # w x w_factor / padded_w
            boxes[:, 4] *= h_factor / padded_h

            targets = torch.zeros((len(boxes),6)) # -> (n, 6)
            targets[:,1:] = boxes
    ...
    def collate_fn(self, batch):
        paths, images, targets = list(zip(*batch)) # zip(*): uncompressed
        #빈것 제외
